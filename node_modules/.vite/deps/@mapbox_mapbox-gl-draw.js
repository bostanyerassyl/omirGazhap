...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/constants.js
...
var LNG_MIN$1 = -270;
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/common_selectors.js
var common_selectors_exports = /* @__PURE__ */ __exportAll({
...
//#endregion
//#region node_modules/wgs84/index.js
var require_wgs84 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports.RADIUS = 6378137;
	module.exports.FLATTENING = 1 / 298.257223563;
	module.exports.POLAR_RADIUS = 6356752.3142;
}));
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/sort_features.js
var import_geojson_area = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function geometry(_) {
		var area = 0, i;
		switch (_.type) {
			case "Polygon": return polygonArea(_.coordinates);
			case "MultiPolygon":
				for (i = 0; i < _.coordinates.length; i++) area += polygonArea(_.coordinates[i]);
				return area;
			case "Point":
			case "MultiPoint":
			case "LineString":
			...
			case "GeometryCollection":
				for (i = 0; i < _.geometries.length; i++) area += geometry(_.geometries[i]);
				return area;
		}
	}
...
function comparator(a, b) {
	...
	if (score === 0 && a.geometry.type === geojsonTypes.POLYGON) return a.area - b.area;
	return score;
}
function sortFeatures(features) {
	return features.map((feature) => {
		if (feature.geometry.type === geojsonTypes.POLYGON) feature.area = import_geojson_area.default.geometry({
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/map_event_to_bounding_box.js
/**
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/string_set.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/features_at.js
var META_TYPES = [
	meta.FEATURE,
	meta.MIDPOINT,
	meta.VERTEX
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/get_features_and_set_cursor.js
function getFeatureAtAndSetCursors(event, ctx) {
	...
	if (ctx.events.currentModeName().indexOf("draw") !== -1) classes.mouse = cursors.ADD;
	ctx.ui.queueMapClasses(classes);
	ctx.ui.updateMapClasses();
	return features[0];
}
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/euclidean_distance.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/is_click.js
...
function isTap(start, end, options = {}) {
	const tolerance = options.tolerance != null ? options.tolerance : 25;
	const interval = options.interval != null ? options.interval : 250;
	start.point = start.point || end.point;
	start.time = start.time || end.time;
	return euclidean_distance_default(start.point, end.point) < tolerance && end.time - start.time < interval;
}
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/mode_handler.js
var ModeHandler = function(mode, DrawContext) {
	...
	const ctx = {
		on(event, selector, fn) {
			if (handlers[event] === void 0) throw new Error(`Invalid event type: ${event}`);
			handlers[event].push({
				selector,
				fn
			});
	...
	const delegate = function(eventName, event) {
		...
		while (iHandle--) {
			...
			if (handle.selector(event)) {
				if (!handle.fn.call(ctx, event)) DrawContext.store.render();
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/node_modules/nanoid/non-secure/index.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/id.js
var nanoid = customAlphabet("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz", 32);
function generateID() {
	return nanoid();
}
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/feature_types/feature.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/feature_types/point.js
...
Point$1.prototype.updateCoordinate = function(pathOrLng, lngOrLat, lat) {
	if (arguments.length === 3) this.coordinates = [lngOrLat, lat];
	else this.coordinates = [pathOrLng, lngOrLat];
	this.changed();
};
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/feature_types/line_string.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/feature_types/polygon.js
...
Polygon.prototype.addCoordinate = function(path, lng, lat) {
	...
	this.coordinates[ids[0]].splice(ids[1], 0, [lng, lat]);
};
Polygon.prototype.removeCoordinate = function(path) {
	...
	if (ring) {
		ring.splice(ids[1], 1);
		if (ring.length < 3) this.coordinates.splice(ids[0], 1);
	}
};
...
Polygon.prototype.updateCoordinate = function(path, lng, lat) {
	...
	if (this.coordinates[ringId] === void 0) this.coordinates[ringId] = [];
	this.coordinates[ringId][coordId] = [lng, lat];
};
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/feature_types/multi_feature.js
var models = {
	MultiPoint: Point$1,
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/mode_interface_accessors.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/mode_interface.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/object_to_mode.js
var eventMapper = {
	drag: "onDrag",
	click: "onClick",
		...
		return {
			start() {
				...
				eventKeys.forEach((key) => {
					...
					let selector = () => false;
					if (modeObject[modeHandler]) selector = () => true;
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/events.js
function events_default(ctx) {
	const modes$1 = Object.keys(ctx.options.modes).reduce((m, k) => {
	...
	events$1.mousedrag = function(event) {
		events$1.drag(event, (endInfo) => !isClick(mouseDownInfo, endInfo));
	};
	events$1.touchdrag = function(event) {
		events$1.drag(event, (endInfo) => !isTap(touchStartInfo, endInfo));
	};
	events$1.mousemove = function(event) {
		if ((event.originalEvent.buttons !== void 0 ? event.originalEvent.buttons : event.originalEvent.which) === 1) return events$1.mousedrag(event);
		event.featureTarget = getFeatureAtAndSetCursors(event, ctx);
	...
	events$1.mouseup = function(event) {
		...
		else currentMode.mouseup(event);
	};
	...
	events$1.touchstart = function(event) {
		...
		event.featureTarget = features_at_default.touch(event, null, ctx)[0];
	...
	events$1.touchend = function(event) {
		...
		else currentMode.touchend(event);
	};
	...
	events$1.keydown = function(event) {
		if (!(event.srcElement || event.target).classList.contains(classes.CANVAS)) return;
		if ((isBackspaceKey(event) || isDeleteKey(event)) && ctx.options.controls.trash) {
			event.preventDefault();
			currentMode.trash();
		} else if (isKeyModeValid(event)) currentMode.keydown(event);
		else if (isDigit1Key(event) && ctx.options.controls.point) changeMode(modes.DRAW_POINT);
		else if (isDigit2Key(event) && ctx.options.controls.line_string) changeMode(modes.DRAW_LINE_STRING);
		else if (isDigit3Key(event) && ctx.options.controls.polygon) changeMode(modes.DRAW_POLYGON);
	};
	events$1.keyup = function(event) {
		if (isKeyModeValid(event)) currentMode.keyup(event);
	};
	...
	events$1.data = function(event) {
		if (event.dataType === "style") {
			...
			if (!options.styles.some((style) => map.getLayer(style.id))) {
				setup.addLayers();
				store.setDirty();
				store.render();
			}
	...
	function changeMode(modename, nextModeOptions, eventOptions = {}) {
		currentMode.stop();
		const modebuilder = modes$1[modename];
		if (modebuilder === void 0) throw new Error(`${modename} is not valid`);
		currentModeName = modename;
		currentMode = ModeHandler(modebuilder(ctx, nextModeOptions), ctx);
		if (!eventOptions.silent) ctx.map.fire(events.MODE_CHANGE, { mode: modename });
		ctx.store.setDirty();
		ctx.store.render();
	}
	const actionState = {
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/to_dense_array.js
/**
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/render.js
function render() {
	const store = this;
	if (!(store.ctx.map && store.ctx.map.getSource(sources.HOT) !== void 0)) return cleanup();
	const mode = store.ctx.events.currentModeName();
	store.ctx.ui.queueMapClasses({ mode });
	let newHotIds = [];
	let newColdIds = [];
	if (store.isDirty) newColdIds = store.getAllIds();
	else {
	...
	function renderFeature(id, source) {
		const featureInternal = store.get(id).internal(mode);
		store.ctx.events.currentModeRender(featureInternal, (geojson) => {
			geojson.properties.mode = mode;
			store.sources[source].push(geojson);
		});
	}
	if (coldChanged) store.ctx.map.getSource(sources.COLD).setData({
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/store.js
function Store(ctx) {
	this._features = {};
	...
	let renderRequest;
	this.render = () => {
		if (!renderRequest) renderRequest = requestAnimationFrame(() => {
			renderRequest = null;
			render.call(this);
...
Store.prototype.featureCreated = function(featureId, options = {}) {
	...
	if ((options.silent != null ? options.silent : this.ctx.options.suppressAPIEvents) !== true) {
		const feature = this.get(featureId);
		this.ctx.events.fire(events.CREATE, { features: [feature.toGeoJSON()] });
	}
	return this;
};
/**
...
Store.prototype.featureChanged = function(featureId, options = {}) {
	this._changedFeatureIds.add(featureId);
	if ((options.silent != null ? options.silent : this.ctx.options.suppressAPIEvents) !== true) this.ctx.events.fire(events.UPDATE, {
...
Store.prototype.delete = function(featureIds, options = {}) {
	...
	toDenseArray(featureIds).forEach((id) => {
		...
		if (!options.silent) {
			if (deletedFeaturesToEmit.indexOf(this._features[id]) === -1) deletedFeaturesToEmit.push(this._features[id].toGeoJSON());
		}
		delete this._features[id];
		this.isDirty = true;
	});
	if (deletedFeaturesToEmit.length) this.ctx.events.fire(events.DELETE, { features: deletedFeaturesToEmit });
...
Store.prototype.getSelectedCoordinates = function() {
	return this._selectedCoordinates.map((coordinate) => {
		return { coordinates: this.get(coordinate.feature_id).getCoordinate(coordinate.coord_path) };
	});
};
/**
...
function refreshSelectedCoordinates(store, options = {}) {
	...
	if (store._selectedCoordinates.length !== newSelectedCoordinates.length && !options.silent) store._emitSelectionChange = true;
...
Store.prototype.storeMapConfig = function() {
	interactions.forEach((interaction) => {
		if (this.ctx.map[interaction]) this._mapInitialConfig[interaction] = this.ctx.map[interaction].isEnabled();
	});
};
/**
...
Store.prototype.restoreMapConfig = function() {
	Object.keys(this._mapInitialConfig).forEach((key) => {
		if (this._mapInitialConfig[key]) this.ctx.map[key].enable();
		else this.ctx.map[key].disable();
	});
};
/**
...
Store.prototype.getInitialConfigValue = function(interaction) {
	if (this._mapInitialConfig[interaction] !== void 0) return this._mapInitialConfig[interaction];
	else return true;
};
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/ui.js
var classTypes = [
	"mode",
	"feature",
	"mouse"
...
function ui_default(ctx) {
	...
	function updateMapClasses() {
		...
		classTypes.forEach((type) => {
			...
			if (nextMapClasses[type] !== null) classesToAdd.push(`${type}-${nextMapClasses[type]}`);
		});
		if (classesToRemove.length > 0) ctx.container.classList.remove(...classesToRemove);
		if (classesToAdd.length > 0) ctx.container.classList.add(...classesToAdd);
	...
	function addButtons() {
		...
		if (controls[types.POINT]) buttonElements[types.POINT] = createControlButton(types.POINT, {
		...
		if (controls[types.LINE]) buttonElements[types.LINE] = createControlButton(types.LINE, {
			container: controlGroup,
		...
		if (controls[types.POLYGON]) buttonElements[types.POLYGON] = createControlButton(types.POLYGON, {
			container: controlGroup,
		...
		if (controls.trash) buttonElements.trash = createControlButton("trash", {
			container: controlGroup,
		...
		if (controls.combine_features) buttonElements.combine_features = createControlButton("combineFeatures", {
			container: controlGroup,
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/setup.js
	...
	const setup = {
		...
		onAdd(map) {
			...
			if (ctx.options.boxSelect) {
				...
				map.dragPan.disable();
				map.dragPan.enable();
				if (!dragPanIsEnabled) map.dragPan.disable();
			}
		...
		removeLayers() {
			...
			if (ctx.map.getSource(sources.COLD)) ctx.map.removeSource(sources.COLD);
			if (ctx.map.getSource(sources.HOT)) ctx.map.removeSource(sources.HOT);
		}
	};
	ctx.setup = setup;
	return setup;
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/theme.js
var blue = "#3bb2d0";
var orange = "#fbb03b";
var white = "#fff";
var theme_default = [
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/mouse_event_point.js
/**
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/create_vertex.js
/**
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/create_midpoint.js
/**
...
function create_midpoint_default(parent, startVertex, endVertex) {
	...
	if (startCoord[1] > 85 || startCoord[1] < -85 || endCoord[1] > 85 || endCoord[1] < -85) return null;
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/create_supplementary_points.js
function createSupplementaryPoints(geojson, options = {}, basePath = null) {
	const { type, coordinates } = geojson.geometry;
	const featureId = geojson.properties && geojson.properties.id;
	let supplementaryPoints = [];
	if (type === geojsonTypes.POINT) supplementaryPoints.push(create_vertex_default(featureId, coordinates, basePath, isSelectedPath(basePath)));
	else if (type === geojsonTypes.POLYGON) coordinates.forEach((line, lineIndex) => {
		processLine(line, basePath !== null ? `${basePath}.${lineIndex}` : String(lineIndex));
	});
	else if (type === geojsonTypes.LINE_STRING) processLine(coordinates, basePath);
	else if (type.indexOf(geojsonTypes.MULTI_PREFIX) === 0) processMultiGeometry();
	function processLine(line, lineBasePath) {
		...
		line.forEach((point, pointIndex) => {
			...
			if (firstPointString !== stringifiedPoint) supplementaryPoints.push(vertex);
			if (pointIndex === 0) firstPointString = stringifiedPoint;
		});
	}
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/double_click_zoom.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/constrain_feature_movement.js
var { LAT_MIN, LAT_MAX, LAT_RENDERED_MIN, LAT_RENDERED_MAX, LNG_MIN, LNG_MAX } = constants_exports;
...
function constrain_feature_movement_default(geojsonFeatures, delta) {
	...
	if (northInnerEdge + constrainedDelta.lat > LAT_RENDERED_MAX) constrainedDelta.lat = LAT_RENDERED_MAX - northInnerEdge;
	if (northOuterEdge + constrainedDelta.lat > LAT_MAX) constrainedDelta.lat = LAT_MAX - northOuterEdge;
	if (southInnerEdge + constrainedDelta.lat < LAT_RENDERED_MIN) constrainedDelta.lat = LAT_RENDERED_MIN - southInnerEdge;
	if (southOuterEdge + constrainedDelta.lat < LAT_MIN) constrainedDelta.lat = LAT_MIN - southOuterEdge;
	if (westEdge + constrainedDelta.lng <= LNG_MIN) constrainedDelta.lng += Math.ceil(Math.abs(constrainedDelta.lng) / 360) * 360;
	if (eastEdge + constrainedDelta.lng >= LNG_MAX) constrainedDelta.lng -= Math.ceil(Math.abs(constrainedDelta.lng) / 360) * 360;
	return constrainedDelta;
}
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/move_features.js
function move_features_default(features, delta) {
	...
	features.forEach((feature) => {
		...
		let nextCoordinates;
		if (feature.type === geojsonTypes.POINT) nextCoordinates = moveCoordinate(currentCoordinates);
		else if (feature.type === geojsonTypes.LINE_STRING || feature.type === geojsonTypes.MULTI_POINT) nextCoordinates = currentCoordinates.map(moveCoordinate);
		else if (feature.type === geojsonTypes.POLYGON || feature.type === geojsonTypes.MULTI_LINE_STRING) nextCoordinates = currentCoordinates.map(moveRing);
		else if (feature.type === geojsonTypes.MULTI_POLYGON) nextCoordinates = currentCoordinates.map(moveMultiPolygon);
		feature.incomingCoords(nextCoordinates);
	});
}
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/simple_select.js
var SimpleSelect = {};
...
SimpleSelect.getUniqueIds = function(allFeatures) {
	...
	return allFeatures.map((s) => s.properties.id).filter((id) => id !== void 0).reduce((memo, id) => {
		memo.add(id);
		return memo;
...
SimpleSelect.stopExtendedInteractions = function(state) {
	...
	if ((state.canDragMove || state.canBoxSelect) && state.initialDragPanState === true) this.map.dragPan.enable();
...
SimpleSelect.onMouseMove = function(state, e) {
	if (isFeature(e) && state.dragMoving) this.fireUpdate();
...
SimpleSelect.clickOnFeature = function(state, e) {
	...
	if (!isShiftClick && isFeatureSelected && this.getFeature(featureId).type !== geojsonTypes.POINT) return this.changeMode(modes.DIRECT_SELECT, { featureId });
	if (isFeatureSelected && isShiftClick) {
		this.deselect(featureId);
		this.updateUIClasses({ mouse: cursors.POINTER });
		if (selectedFeatureIds.length === 1) double_click_zoom_default.enable(this);
...
SimpleSelect.onCombineFeatures = function() {
	...
	for (let i = 0; i < selectedFeatures.length; i++) {
		...
		if (feature.type.includes("Multi")) feature.getCoordinates().forEach((subcoords) => {
			coordinates.push(subcoords);
		});
		else coordinates.push(feature.getCoordinates());
...
SimpleSelect.onUncombineFeatures = function() {
	...
	if (createdFeatures.length > 1) this.fire(events.UNCOMBINE_FEATURES, {
		createdFeatures,
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/direct_select.js
...
DirectSelect.startDragging = function(state, e) {
	if (state.initialDragPanState == null) state.initialDragPanState = this.map.dragPan.isEnabled();
	this.map.dragPan.disable();
	state.canDragMove = true;
	state.dragMoveLocation = e.lngLat;
};
DirectSelect.stopDragging = function(state) {
	if (state.canDragMove && state.initialDragPanState === true) this.map.dragPan.enable();
...
DirectSelect.onVertex = function(state, e) {
	...
	if (!isShiftDown(e) && selectedIndex === -1) state.selectedCoordPaths = [about.coord_path];
	else if (isShiftDown(e) && selectedIndex === -1) state.selectedCoordPaths.push(about.coord_path);
...
DirectSelect.dragVertex = function(state, e, delta) {
	...
	const constrainedDelta = constrain_feature_movement_default(selectedCoords.map((coords) => ({
		type: geojsonTypes.FEATURE,
		properties: {},
		geometry: {
			type: geojsonTypes.POINT,
...
DirectSelect.onSetup = function(opts) {
	...
	if (feature.type === geojsonTypes.POINT) throw new TypeError("direct_select mode doesn't handle point features");
	const state = {
		featureId,
		feature,
...
DirectSelect.onMouseMove = function(state, e) {
	...
	if ((onVertex || isFeature || isMidPoint) && state.dragMoving) this.fireUpdate();
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/draw_point.js
var DrawPoint = {};
...
DrawPoint.onTap = DrawPoint.onClick = function(state, e) {
	...
	this.fire(events.CREATE, { features: [state.point.toGeoJSON()] });
	this.changeMode(modes.SIMPLE_SELECT, { featureIds: [state.point.id] });
};
DrawPoint.onStop = function(state) {
	this.activateUIButton();
	if (!state.point.getCoordinate().length) this.deleteFeature([state.point.id], { silent: true });
};
...
DrawPoint.onKeyUp = function(state, e) {
	if (isEscapeKey(e) || isEnterKey(e)) return this.stopDrawingAndRemove(state, e);
};
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/is_event_at_coordinates.js
function isEventAtCoordinates(event, coordinates) {
	if (!event.lngLat) return false;
	return event.lngLat.lng === coordinates[0] && event.lngLat.lat === coordinates[1];
}
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/draw_polygon.js
var DrawPolygon = {};
...
DrawPolygon.clickAnywhere = function(state, e) {
	if (state.currentVertexPosition > 0 && isEventAtCoordinates(e, state.polygon.coordinates[0][state.currentVertexPosition - 1])) return this.changeMode(modes.SIMPLE_SELECT, { featureIds: [state.polygon.id] });
	this.updateUIClasses({ mouse: cursors.ADD });
...
DrawPolygon.onMouseMove = function(state, e) {
	state.polygon.updateCoordinate(`0.${state.currentVertexPosition}`, e.lngLat.lng, e.lngLat.lat);
	if (isVertex$1(e)) this.updateUIClasses({ mouse: cursors.POINTER });
};
...
DrawPolygon.onKeyUp = function(state, e) {
	if (isEscapeKey(e)) {
		...
		this.changeMode(modes.SIMPLE_SELECT);
	} else if (isEnterKey(e)) this.changeMode(modes.SIMPLE_SELECT, { featureIds: [state.polygon.id] });
};
DrawPolygon.onStop = function(state) {
	...
	if (state.polygon.isValid()) this.fire(events.CREATE, { features: [state.polygon.toGeoJSON()] });
	else {
		this.deleteFeature([state.polygon.id], { silent: true });
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/draw_line_string.js
var DrawLineString = {};
DrawLineString.onSetup = function(opts) {
	...
	if (featureId) {
		...
		let from = opts.from;
		if (from && from.type === "Feature" && from.geometry && from.geometry.type === "Point") from = from.geometry;
		if (from && from.type === "Point" && from.coordinates && from.coordinates.length === 2) from = from.coordinates;
		if (!from || !Array.isArray(from)) throw new Error("Please use the `from` property to indicate which point to continue the line from");
...
DrawLineString.clickAnywhere = function(state, e) {
	if (state.currentVertexPosition > 0 && isEventAtCoordinates(e, state.line.coordinates[state.currentVertexPosition - 1]) || state.direction === "backwards" && isEventAtCoordinates(e, state.line.coordinates[state.currentVertexPosition + 1])) return this.changeMode(modes.SIMPLE_SELECT, { featureIds: [state.line.id] });
	this.updateUIClasses({ mouse: cursors.ADD });
	state.line.updateCoordinate(state.currentVertexPosition, e.lngLat.lng, e.lngLat.lat);
	if (state.direction === "forward") {
		state.currentVertexPosition++;
		state.line.updateCoordinate(state.currentVertexPosition, e.lngLat.lng, e.lngLat.lat);
	} else state.line.addCoordinate(0, e.lngLat.lng, e.lngLat.lat);
};
...
DrawLineString.onKeyUp = function(state, e) {
	if (isEnterKey(e)) this.changeMode(modes.SIMPLE_SELECT, { featureIds: [state.line.id] });
...
DrawLineString.onStop = function(state) {
	...
	if (state.line.isValid()) this.fire(events.CREATE, { features: [state.line.toGeoJSON()] });
	else {
		this.deleteFeature([state.line.id], { silent: true });
		this.changeMode(modes.SIMPLE_SELECT, {}, { silent: true });
	}
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/modes/index.js
var modes_default = {
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/options.js
var defaultOptions = {
...
function options_default(options = {}) {
	...
	if (!options.controls) withDefaults.controls = {};
	if (options.displayControlsDefault === false) withDefaults.controls = Object.assign({}, hideControls, options.controls);
	else withDefaults.controls = Object.assign({}, showControls, options.controls);
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/string_sets_are_equal.js
var import_geojson_normalize = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	function normalize(gj) {
		...
		else if (type === "feature") return {
			type: "FeatureCollection",
			features: [gj]
		};
		else if (type === "featurecollection") return gj;
	}
})))(), 1);
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/api.js
var featureTypes = {
	Polygon,
	LineString,
	Point: Point$1,
...
function api_default(ctx, api) {
	...
	api.getFeatureIdsAt = function(point) {
		return features_at_default.click({ point }, null, ctx).map((feature) => feature.properties.id);
	};
	...
	api.set = function(featureCollection) {
		if (featureCollection.type === void 0 || featureCollection.type !== geojsonTypes.FEATURE_COLLECTION || !Array.isArray(featureCollection.features)) throw new Error("Invalid FeatureCollection");
		...
		if (toDelete.length) api.delete(toDelete);
		renderBatch();
		return newIds;
	};
	api.add = function(geojson) {
		const ids = JSON.parse(JSON.stringify((0, import_geojson_normalize.default)(geojson))).features.map((feature) => {
			feature.id = feature.id || generateID();
			if (feature.geometry === null) throw new Error("Invalid geometry: null");
			if (ctx.store.get(feature.id) === void 0 || ctx.store.get(feature.id).type !== feature.geometry.type) {
				...
				if (Model === void 0) throw new Error(`Invalid geometry type: ${feature.geometry.type}.`);
				...
				if (!(0, import_fast_deep_equal.default)(originalProperties, feature.properties)) ctx.store.featureChanged(internalFeature.id, { silent });
				if (!(0, import_fast_deep_equal.default)(internalFeature.getCoordinates(), feature.geometry.coordinates)) internalFeature.incomingCoords(feature.geometry.coordinates);
			}
			return feature.id;
		});
		ctx.store.render();
	...
	api.get = function(id) {
		...
		if (feature) return feature.toGeoJSON();
	};
	api.getAll = function() {
		return {
			type: geojsonTypes.FEATURE_COLLECTION,
	...
	api.delete = function(featureIds) {
		...
		if (api.getMode() === modes.DIRECT_SELECT && !ctx.store.getSelectedIds().length) ctx.events.changeMode(modes.SIMPLE_SELECT, void 0, { silent });
		else ctx.store.render();
		return api;
	};
	api.deleteAll = function() {
		ctx.store.delete(ctx.store.getAllIds(), { silent });
		if (api.getMode() === modes.DIRECT_SELECT) ctx.events.changeMode(modes.SIMPLE_SELECT, void 0, { silent });
		else ctx.store.render();
		return api;
	};
	api.changeMode = function(mode, modeOptions = {}) {
		...
		if (mode === modes.DIRECT_SELECT && api.getMode() === modes.DIRECT_SELECT && modeOptions.featureId === ctx.store.getSelectedIds()[0]) return api;
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/src/lib/index.js
...
//#endregion
//#region node_modules/@mapbox/mapbox-gl-draw/index.js
